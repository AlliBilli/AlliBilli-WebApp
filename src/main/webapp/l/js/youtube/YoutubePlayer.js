 ...
 * @version 0.3
 */
Ext.ux.YoutubePlayer = Ext.extend(Ext.flash.Component, {
    ...
    _delegateErrorEvent : function(errorCode)
    {
        switch (errorCode) {
            ...
            case 2:  errorCode = 'Invalid Id';  break;
            case 150:  this.nextVideo(); break;
...
//-------------------------- Youtube API
    ...
    cuePlaylist : function()
    {
    	...
    	this.player.cuePlaylist(uTubePlayListId, randomnumber);
    },
    nextVideo:function()
    {
...
var _onYouTubePlayerReady = function(playerId) {
	 panel = Ext.ux.YoutubePlayer.Players[playerId];
    if (panel) {
        var player = document.getElementById(playerId);
        panel._initPlayer(player);
        player.addEventListener('onStateChange', "execStateChangeEvt");
        player.addEventListener('onError', "execErrorEvt");
...
function execStateChangeEvt(state)
{
	youtubePlayerPanel._delegateStateEvent(state);
}
function execErrorEvt(errCode)
{
	youtubePlayerPanel._delegateErrorEvent(errCode);;
}
