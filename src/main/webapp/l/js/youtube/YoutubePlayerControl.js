 ...
 */
Ext.ux.YoutubePlayer.Control = Ext.extend(Ext.toolbar.Toolbar, {
    ...
    _parseVideoId : function(text)
    {
        ...
        if((text == null || text.length > 11 )&& aText.length > 11 && aText.charAt(0) == "P" && aText.charAt(1) == "L")
        	{
        		text = aText.slice(2, aText.length);
        	}
        alert(text);
        return text;
    },
    ...
    {
        this.player.seekTo(this.sliderField.getValue(),this.isSeekTo);
    ...
    _onMuteToggle : function(button)
    {
        ...
        if (pressed) {
            button.setIconCls('ext-ux-youtubeplayer-control-muteIcon');
            if (isMore) {
            ...
            button.setIconCls('ext-ux-youtubeplayer-control-volumeIcon');
            if (isMore) {
                this.muteButton.toggle(false);
                return;
            }
    ...
    initComponent : function()
    {
       // var tb = Ext.Toolbar.Button;
        ...
        this.ejectButton = new tb({
            iconCls  : 'eject',
            tooltip:'Loads a Playlist or a Vedio',
            disabled : true
        });
        ...
        this.teluguRyhmesItem = new Ext.menu.CheckItem({
          text     : 'Play Telugu Ryhmes',
          group    : 'kids',
          disabled : false
        });
        this.tamilRyhmesItem = new Ext.menu.CheckItem({
          text     : 'Play Tamil Ryhmes',
          group    : 'kids',
          disabled : false
        });
       ...
       /* this.choosePlayList = new Ext.Button({
          iconCls : 'quality',
          menu    : [
              this.teluguRyhmesItem,
              '-',
              this.tamilRyhmesItem
          ]
      });*/
        ...
        this.volumeSlider = new Ext.Slider({
            ...
            disabled : true,
            style:{
            	backgroundColor : (Ext.isIE)?'#CCCCCC':'none'
            }
        });
        ...
        this.sliderField = Ext.create('Ext.slider.Single', {
          ...
          useTips: false,
          style:{
          	backgroundColor : (Ext.isIE)?'#CCCDDC':'none'
          }
      });
       ...
       /* this.sliderField = new Ext.ux.YoutubePlayer.Control.Slider({
		...
		this.muteButton = new  Ext.button.Button({
    ...
    _initListeners : function()
    {
       ...
       /* this.on('afterlayout', function() {
        ...
        this.previousButton.on('click', this._onPrevious, this);
        ...
        var c = this;
        this.player.on('stateChange', function(state, panel, player)
        		{
        			if(state && panel && player)
        				c._processPlayerEvents(state, panel, player);
        		}, this);
        this.sliderField.on('dragstart', function(){this.isAdjusting = true; this.isSeekTo = false;}, this);
        this.sliderField.on('drag', this._onSeekPosition, this);
        this.sliderField.on('dragend', function(){this.isAdjusting = false; this.isSeekTo = true;}, this);
        ...
        //this.teluguRyhmesItem.on('checkchange', this._onPlayListItemCheckChange, this);
        //this.tamilRyhmesItem.on('checkchange', this._onPlayListItemCheckChange, this);
    },
    ...
    _updateAvailableQualityLevels : function(levels)
    {
    	for(var key in levels) {
    	  if (levels[key] == 'small') {this.qualitySmallItem.setDisabled(false);}
    	  if (levels[key] == 'medium') {this.qualityMediumItem.setDisabled(false);}
      	if (levels[key] == 'large') {this.qualityLargeItem.setDisabled(false);}
        if (levels[key] == 'hd720') {this.qualityHd720Item.setDisabled(false);}
   	 }
    ...
    {
        var control = this;
        Ext.Msg.prompt('Load a video or a playlist', 'Please enter the video id or playList id:', function(btn, text){
            if (btn == 'ok'){
                var id = control._parseVideoId(text);
                if (!id) {
                    Ext.Msg.alert(
                        'Load video/playList',
                        "id \""+id+"\" does not seem to be a valid"
                    );
                    return;
                }
                ...
                if(id.length > 11)
                	{
                		uTubePlayListId = id;
                		control.player.cuePlaylist(uTubePlayListId);
                	}
                else
    ...
    _loadCustomPlayList : function (id)
    {
    ...
    _onPlayListItemCheckChange : function(item, checked)
    {
    	var control = this;
    	
    	var id = control._parseVideoId("PL2EBED2C9F31DF80D");
      if (!id) {
          Ext.Msg.alert(
              'Load video/playList',
              "id \""+id+"\" does not seem to be a valid"
      ...
      control.player.clearVideo();
    	...
    	switch (item) {
        case this.teluguRyhmesItem:
        	control.player.cuePlaylist(id);
        break;
        case this.tamilRyhmesItem:
        	control.player.cuePlaylist(id);
        break;
    	}
    },
    ...
    _updateVideoInfo : function(ignorePaused)
    {
        ...
        if (totalTime != 0) {
            ...
            this.sliderField.setMaxValue(totalTime);
    ...
    _processPlayerEvents : function(state, panel, player)
    {
        switch (state) {
            ...
            case 'ended':
                ...
                this.playButton.setIconCls('play');
            ...
            case 'playing':
                ...
                this.playButton.setIconCls('pause');
            break;
